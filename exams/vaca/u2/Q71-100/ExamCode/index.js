...
function getCollection(client) {
    return client.db('OOP').collection('ACME');
}

// Unified Building lookup by _id
async function getBuildingById(client, id) {
...
app.get('/building/:id', async (req, res) => {
    ...
    try {
        client = await connectToDatabase();
        const player = await getBuildingById(client, req.params.id);
