...
public class FrmContact extends javax.swing.JFrame {
    ...
    private boolean isValidGmail(String email) {
        Pattern p = Pattern.compile("^[a-zA-Z0-9._%+-]+@gmail\\.com$", Pattern.CASE_INSENSITIVE);
        Matcher m = p.matcher(email);
        return m.matches();
    }

    private boolean isValidPhoneNumber(String phone) {
        Pattern p = Pattern.compile("^(09\\d{8}|0[2-7]\\d{7})$");
    ...
    private void btmSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btmSaveActionPerformed
        ...
        String phone = txtCellphoneNumber.getText().trim();
        ...
        try {
            ...
            if (!isValidBirthDate(birthDate)) {
                JOptionPane.showMessageDialog(this, "Date incorrect", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (ParseException e) {
            JOptionPane.showMessageDialog(this, "Enter Date Correct", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        SimpleDateFormat sdfOutput = new SimpleDateFormat("dd/MM/yyyy");
        String birthDateFormatted = sdfOutput.format(birthDate);
        JOptionPane.showMessageDialog(this, "SAVE correct", "Éxito", JOptionPane.INFORMATION_MESSAGE);
